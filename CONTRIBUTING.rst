...
Everyone interacting in the PyPkg M7RAP project's code base,
...
Report bugs at https://github.com/goerz/pypkg_m7rap/issues.
...
The best way to send feedback is to file an issue at https://github.com/goerz/pypkg_m7rap/issues.
...
3. Check https://travis-ci.org/goerz/pypkg_m7rap/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``goerz/pypkg_m7rap`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:goerz/pypkg_m7rap.git``
...
PyPkg M7RAP uses a simple branching model where all
...
PyPkg M7RAP includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``pypkg_m7rap`` repository  you can use
...
.. _test coverage: https://coveralls.io/github/goerz/pypkg_m7rap?branch=master
...
PyPkg M7RAP could always use more documentation, whether
...
At any point, from a checkout of the ``pypkg_m7rap``
...
:mod:`pypkg_m7rap` package:

.. doctest::

    >>> import pypkg_m7rap
    >>> pypkg_m7rap.__version__   # doctest: +SKIP
...
https://github.com/goerz/pypkg_m7rap/releases.
